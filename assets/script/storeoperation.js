...
cc.Class({
    ...
    start () {
        let operation0 = function(type, card_name, card_kind){
            if (type == '0'){ // 牌库里抽
                ...
                action.push(false);
                let index_now = action.length-1;
                let seq = cc.sequence(cc.moveTo(0.1, 99, 43), cc.callFunc(function(target, index){
                    finished();
                    action[index] = true;
                }, this, index_now));
                card_now.zIndex = epoch++;
        ...
        let operation1 = function(type, card_name, card_kind){
            if (type == '1') {
                let card_now = cc.find("Canvas/New Sprite(Splash)/"+card_name);
